...
h.set_labels(options.button_labels);
h.disable_buttons();
drawnow()
...
while mean([expe.( phase ).conditions.done])~=1 % Keep going while there are some conditions to do
    ...
    h.show_instruction();
    h.show_buttons();
    h.set_instruction(sprintf(options.instructions.task));
    ...
    while true
        ...
        if length(decision_vector)>=options.(phase).down_up(1) && all(decision_vector(end-(options.(phase).down_up(2)-1):end)==1)
            ...
            fprintf('--> Where going down by %f Hz\n', step_size);
        ...
        elseif length(steps)>6 && all(steps(end-5:end)==0)
             ...
             fprintf('====> END OF RUN too easy in floor condition\n');
