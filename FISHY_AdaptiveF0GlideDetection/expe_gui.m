...
% Creates the gui. Called from expe_main().
...
% This needs to be adjusted if you want the GUI to be displayed on a
% secondary screen.
...
n_cols = options.n_intervals; % options.n_cols;
...
h.instruction = uicontrol('Style', 'text', 'Units', 'pixel', 'Position', [50, height/2-grid_sz(2)/2+grid_sz(2)+10, width-50*2, 50], ...
...
h.set_labels = @(labels) set_labels(h, labels);
...
%--------------------------------------------------------------------------
function set_labels(h, labels)

for i=1:length(h.t(:))
    set(h.t(i), 'String', labels{i});
end
