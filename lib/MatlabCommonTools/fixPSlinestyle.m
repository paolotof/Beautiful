...
if length(varargin)>2
    lines = varargin{3};
    if isfield(lines, 'solidLine')
        solidLine = lines.solidLine;
        fprintf('Using this definition for "solidLine":\n   %s\n', solidLine);
    end
    if isfield(lines, 'dotLine')
        dotLine = lines.dotLine;
        fprintf('Using this definition for "dotLine":\n   %s\n', dotLine);
    end
    if isfield(lines, 'dashedLine')
        dashedLine = lines.dashedLine;
        fprintf('Using this definition for "dashedLine":\n   %s\n', dashedLine);
    end
    if isfield(lines, 'dashdotLine')
        dashdotLine = lines.dashdotLine;
        fprintf('Using this definition for "dashdotLine":\n   %s\n', dashdotLine);
    end
end
